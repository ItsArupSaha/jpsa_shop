...
export function DownloadSaleMemo({ sale, customer, items, user }: DownloadSaleMemoProps) {
  ...
  const generatePdf = () => {
    ...
    const footContent = [
        [{ content: 'Subtotal', colSpan: 3, styles: { halign: 'right' } }, `BDT ${sale.subtotal.toFixed(2)}`],
        [{ content: 'Discount', colSpan: 3, styles: { halign: 'right' } }, `-BDT ${(sale.subtotal - sale.total).toFixed(2)}`],
        [{ content: 'Grand Total', colSpan: 3, styles: { halign: 'right', fontSize: 12 } }, `BDT ${sale.total.toFixed(2)}`],
    ];

    if (sale.paymentMethod === 'Split') {
        const dueAmount = sale.total - (sale.amountPaid || 0);
        footContent.push(
            [{ content: 'Amount Paid', colSpan: 3, styles: { halign: 'right' as const } }, `BDT ${sale.amountPaid?.toFixed(2)}`],
            [{ content: 'Amount Due', colSpan: 3, styles: { halign: 'right' as const } }, `BDT ${dueAmount.toFixed(2)}`]
        );
    }
     if (sale.paymentMethod === 'Due') {
        footContent.push(
            [{ content: 'Amount Due', colSpan: 3, styles: { halign: 'right' as const } }, `BDT ${sale.total.toFixed(2)}`]
        );
    }
